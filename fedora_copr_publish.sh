...
version_in_changelog=$(grep -E "system76-firmware \([[:digit:]]+\.[[:digit:]]+\.[[:digit:]]+" debian/changelog | head -1)
_tmp=${version_in_changelog%%)*}
_tmp=${_tmp%%~*}
version=${_tmp##*\(}

_tmp=$(git tag --list system76-firmware-"$version"-'*' | sort -n -t '-' -k 4 -r | head -1)
release=${_tmp##*-}
